...
declare_id!("6crw656Yy1M7AMf9eymEthcPGb7WNHH63a1m8Ben8ynJ");
...
pub mod tic_tac_toe {
    ...
    pub fn play(ctx: Context<Play>, tile: Tile) -> Result<()> {
        let game = &mut ctx.accounts.game;
        require_keys_eq!(game.current_player(), ctx.accounts.player.key(), TicTacToeError::NotPlayersTurn);
        game.play(&tile)
    }
}
...
pub struct SetupGame<'info> {
    ...
    #[account(seeds = [b"game", player_one.key().as_ref(), _game_id.as_bytes()])]
    #[account(bump)]
